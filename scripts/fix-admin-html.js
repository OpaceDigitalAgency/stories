...
async function findFilesToFix() {
  const result = require('child_process').execSync(
    'grep -r "<head" --include="*.php" stories-backend/admin/',
    { encoding: 'utf-8' }
  );
...
async function fixFile(filePath) {
  try {
    ...
    const phpStartMatch = content.match(/^<\?php([\s\S]*?)(?=<!DOCTYPE|<html|$)/);
    let phpStart = '';
    if (phpStartMatch) {
    ...
    if (bodyMatch) {
      ...
      console.log(`Skipping ${filePath} - could not extract body content`);
      return;
    }
    ...
    // Extract the current page variable if it exists
    const currentPageMatch = content.match(/\$currentPage\s*=\s*['"]([^'"]+)['"]/);
    let currentPage = path.basename(filePath, '.php');
    if (currentPageMatch) {
      currentPage = currentPageMatch[1];
    }
