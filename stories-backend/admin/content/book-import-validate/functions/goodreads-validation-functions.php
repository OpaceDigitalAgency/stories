 ...
 */
function fetchGoodreadsData($isbn, $title, $author) {
    try {
        ...
        return fetchGoodreadsDataWithCurl($isbn, $title, $author, $searchUrl);
    } catch (Exception $e) {
 ...
 * @return array|null Book data or null if not found
 */
function fetchGoodreadsDataWithCurl($isbn, $title, $author, $searchUrl) {
    try {
        $startTime = microtime(true);
