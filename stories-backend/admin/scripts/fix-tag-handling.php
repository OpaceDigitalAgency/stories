...
try {
    ...
    if (count($ageTags) > 0) {
        ...
        foreach ($ageTags as $tag) {
            ...
            $stmt = $db->prepare("
                SELECT b.id, b.directory_item_id, b.age_range
                FROM books b
            ...
            if (count($books) > 0) {
                ...
                foreach ($books as $book) {
                    if (empty($book['age_range'])) {
                        $stmt = $db->prepare("UPDATE books SET age_range = ? WHERE id = ?");
                        $stmt->execute([$tagName, $book['id']]);
                        output("  - Updated book ID " . $book['id'] . " with age_range '" . $tagName . "'", $isWeb);
                    }
                }
            }
        }
