<?php
/**
 * Force Add Default Prompt Templates
 * 
 * This script forces the addition of default prompt templates for AI image generation,
 * even if they already exist. It will replace any existing templates with the same name.
 * 
...
<head>
    ...
    <title>Force Add AI Prompt Templates</title>
    <style>
        body {
...
<body>
    <h1>Force Add AI Prompt Templates</h1>
    
    <div class="log">
<?php
// Debug database connection
echo "<div class='debug'>Database connection: " . (isset($db) ? "Established" : "Failed") . "</div>\n";
if (isset($db)) {
    try {
        $testQuery = $db->query("SELECT 1");
        echo "<div class='debug'>Database test query: Success</div>\n";
    } catch (PDOException $e) {
        echo "<div class='debug'>Database test query failed: " . htmlspecialchars($e->getMessage()) . "</div>\n";
    }
}
...
try {
    ...
    if (!$tableExists) {
        ...
        echo "<div class='info'>Table ai_prompt_templates already exists.</div>\n";
        
        // Show current table structure
        $stmt = $db->query("DESCRIBE ai_prompt_templates");
        $columns = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo "<div class='debug'>Table structure:<br>";
        foreach ($columns as $column) {
            echo htmlspecialchars($column['Field'] . " - " . $column['Type']) . "<br>";
        }
        echo "</div>\n";
    }

    // Force add templates for each content type
    foreach ($defaultTemplates as $template) {
        $contentType = $template['content_type'];
        $name = $template['name'];
        
        echo "<div class='info'>Processing template: $name for content type: $contentType</div>\n";
        
        // Check if template already exists
        $stmt = $db->prepare("SELECT id FROM ai_prompt_templates WHERE content_type = ? AND name = ?");
        $stmt->execute([$contentType, $name]);
        $existingId = $stmt->fetchColumn();
        
        if ($existingId) {
            // Update existing template
            echo "<div class='info'>Template already exists (ID: $existingId). Updating...</div>\n";
            
            $stmt = $db->prepare("
                UPDATE ai_prompt_templates 
                SET description = ?, prompt_template = ?, is_active = ?
                WHERE id = ?
            ");
            ...
            echo "<div class='success'>Updated template for $contentType: $name</div>\n";
        } else {
            // Add new template
            echo "<div class='info'>Template does not exist. Adding new template...</div>\n";
            ...
            try {
                ...
                echo "<div class='success'>Added template for $contentType: $name</div>\n";
            } catch (PDOException $e) {
                echo "<div class='error'>Error adding template: " . htmlspecialchars($e->getMessage()) . "</div>\n";
            }
        }
    }
    ...
    foreach ($contentTypes as $contentType) {
        ...
        echo "<div class='info'>Verified $count templates for content type: $contentType</div>\n";
    }
    
    echo "<div class='success'>Default prompt templates force-added successfully.</div>\n";
